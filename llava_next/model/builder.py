...
import torch
from llava_next.model import *
from llava_next.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if load_8bit:
        ...
        kwargs["torch_dtype"] = torch.float16 if "torch_dtype" not in kwargs else kwargs["torch_dtype"]
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                from llava_next.model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                from llava_next.model.language_model.llava_mistral import LlavaMistralConfig
                ...
                from llava_next.model.language_model.llava_gemma import LlavaGemmaConfig
                ...
                if "moe" in model_name.lower():
                    from llava_next.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    ...
                    from llava_next.model.language_model.llava_qwen import LlavaQwenConfig
                ...
                from llava_next.model.language_model.llava_llama import LlavaConfig
            ...
            if 'paidge' in model_name.lower():
                if isinstance(model, LlavaQwenForCausalLM):
                    from llava_next.model.language_model.paidge_qwen import PaidgeQwen2ForCausalLM
                    model = PaidgeQwen2ForCausalLM(model)
                elif isinstance(model, LlavaLlamaForCausalLM):
                    from llava_next.model.language_model.paidge_llama import PaidgeLlamaForCausalLM
                    model = PaidgeLlamaForCausalLM(model)
                else:
                    raise ValueError(f"Model {model_name} not supported")
                # model = PaidgeModelV3ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
                # image_kv_parameter = torch.load("./checkpoints_out/pretrain/Pretrain_llava1.5_paidge_qwen2-1.5B_CLIP336_cross_decouple_6layers/checkpoint-2180/other_parameters.bin", map_location="cpu", weights_only=True)
                # model.load_state_dict(image_kv_parameter, strict=False)
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower():
                    from llava_next.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    cfg_pretrained = LlavaQwenMoeConfig.from_pretrained(model_path)
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    model = LlavaQwenMoeForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
                ...
                or "mobilellm" in model_name.lower()
            ...
            if "mixtral" in model_name.lower():
                ...
                if "paidge" in model_name.lower():
                    from llava_next.model.language_model.paidge_llama import PaidgeLlamaForCausalLM
                    model = PaidgeLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                else:
                    from llava_next.model.language_model.llava_llama import LlavaConfig
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if 'flamingo' in model_name.lower():
                        from llava_next.model.language_model.famingo import FlamingoModel
                        model = FlamingoModel.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                    elif 'promptaware' in model_name.lower():
                        from llava_next.model.language_model.prompt_aware import PromptAwareModelForCausalLM
                        model = PromptAwareModelForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                    elif "paidge" in model_name.lower():
                        from llava_next.model.language_model.paidge_qwen import PaidgeQwen2ForCausalLM
                        model = PaidgeQwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                    else:
                        from llava_next.model.language_model.llava_qwen import LlavaQwenConfig
                        if overwrite_config is not None:
