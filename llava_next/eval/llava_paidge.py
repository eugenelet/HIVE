...
@register_model("llava_paidge")
class llava_paidge(lmms):
    """
    Llava Model
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self._tokenizer, self._model, self._image_processor, self._max_length = load_pretrained_model(model_path, model_base, model_name, device_map=self.device_map, **llava_model_args)
        except TypeError:
    ...
    def generate_until(self, requests):
        ...
        for chunk in chunks:
            ...
            if "promptaware" in type(self.model).__name__.lower():
                # contexts = self.model.text_tower.tokenizer(contexts, truncation=True, max_length=77, return_tensors="pt")
                contexts = self.tokenizer(contexts, truncation=True, max_length=77, return_tensors="pt")
                for key in contexts.keys():
                    contexts[key] = contexts[key].to(self.device)
            ...
            try:
                cont = self.model.generate(
                    ...
                    prompts=contexts if "promptaware" in type(self.model).__name__.lower() else None,
